...
const ToolsPage = () => {
  ...
  const [tempSelectedImageTool, setTempSelectedImageTool] =
    useState<string>("");

  // Update temp selection when the actual selection loads
  useEffect(() => {
    if (selectedImageTool?.selected_tool && !tempSelectedImageTool) {
      setTempSelectedImageTool(selectedImageTool.selected_tool);
    }
  }, [selectedImageTool?.selected_tool, tempSelectedImageTool]);
  ...
  const handleTempImageToolSelectionChange = (e: RadioChangeEvent) => {
    setTempSelectedImageTool(e.target.value);
  };

  const handleSaveImageToolSelection = () => {
    if (tempSelectedImageTool) {
  ...
  const hasSelectionChanged =
    tempSelectedImageTool !== selectedImageTool?.selected_tool;
