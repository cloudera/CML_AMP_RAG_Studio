...
class TestPaginate:
    def test_paginate_with_limit_only(self) -> None:
        ...
        paginated, previous_id, next_id = paginate(results, limit=3, offset=None)
        assert paginated == [3, 4, 5]

        # Get the last 1 item
        paginated, previous_id, next_id = paginate(results, limit=1, offset=None)
        assert paginated == [5]

        # Get all items (limit larger than list)
        paginated, previous_id, next_id = paginate(results, limit=10, offset=None)
    ...
    def test_paginate_with_limit_and_offset(self) -> None:
        ...
        paginated, previous_id, next_id = paginate(results, limit=3, offset=2)
        assert paginated == [6, 7, 8]
        assert previous_id == -1 ## is this ok?
        assert next_id == 5

        # Get 2 items, offset 5 from the end
        # Should return items [4, 5] (skipping 6, 7, 8, 9, 10)
        paginated, previous_id, next_id = paginate(results, limit=2, offset=5)
        assert paginated == [4, 5]
        assert previous_id == 3
        assert next_id == 7

        # Get 4 items, offset 0 from the end (same as limit only)
        paginated, previous_id, next_id = paginate(results, limit=4, offset=0)
        assert paginated == [7, 8, 9, 10]
        assert previous_id is None
        assert next_id == 4

        with raises(ValueError):
            paginate(results, limit=3, offset=-1)

        with raises(ValueError):
            paginate(results, limit=-3, offset=2)
    ...
    def test_paginate_edge_cases(self) -> None:
        ...
        # Empty list
        results: list[Any] = []
        paginated, previous_id, next_id = paginate(results, limit=5, offset=None)
        assert paginated == []

        paginated, previous_id, next_id = paginate(results, limit=5, offset=2)
        assert paginated == []
        ...
        results = [1, 2, 3, 4, 5]
        paginated, previous_id, next_id = paginate(results, limit=3, offset=3)
        assert paginated == [1, 2]

        # None values for both limit and offset
        paginated, previous_id, next_id = paginate(results, limit=None, offset=None)
        ...
        paginated, previous_id, next_id = paginate(results, limit=None, offset=2)
    ...
    def test_paginate_with_different_types(self) -> None:
        ...
        paginated, previous_id, next_id = paginate(results, limit=2, offset=1)
