...
class DataSourceConfiguration(BaseModel):
    ...
    """ # noqa: E501
    connection_type: StrictStr = Field(description="Data source connection type")
    chunk_size: StrictInt = Field(description="Data source chunk size")
    chunk_overlap_percent: StrictInt = Field(description="Data source chunk overlap percentage")
    __properties: ClassVar[List[str]] = ["connection_type", "chunk_size", "chunk_overlap_percent"]

    @field_validator('connection_type')
    def connection_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('MANUAL', 'CDF', 'API', 'OTHER'):
            raise ValueError("must be one of enum values ('MANUAL', 'CDF', 'API', 'OTHER')")
        return value
